"""(HBr)2 dimer: classification of the April 2026 CNEO-DFT runs (aug-cc-pV5Z excluded).
...
out = []
w = out.append
w("(HBr)2 dimer, CNEO-DFT April 2026 runs: classification (aug-cc-pV5Z excluded)")
w("Masses: PySCF isotope-averaged (H 1.008, Br 79.904). Atom order in every log: H0 Br1 H2 Br3 (H0-Br1 and H2-Br3 the HBr units); donor read from the distances.")
w("")
w("PART 0: run status")
...
w("  r(Br-H)d = donor Br-H, r(H..Br) = hydrogen bond, r(Br-H)a = acceptor Br-H, r(Br..Br); donor = the H closer to the other Br;")
w("  angles: Br-H...Br (linearity of the H bond), H...Br-H (tilt of the acceptor); q = role of the quantum proton.")
w(f"{'start':<11}{'xc':<6}{'basis':<13}{'E (Eh)':>16} {'Br-Hd':>6} {'H..Br':>6} {'Br-Ha':>6} {'Br..Br':>6} {'BrH.Br':>7} {'H.BrH':>7} {'q':>9}  "
  f"{'Br sum rule':>11}  vibrations (cm^-1), 3N-6 = 6            imag  class")
class_summary = []
for start in ("global_min", "local_min"):
    for bas in BAS_ORDER:
        for xc in FUNC_ORDER:
...
w("")
w("PART 2: external modes and the bromine sum-rule defect")
w("  'full 3N' is the unprojected spectrum: 6 external modes should be ~0 but are displaced by the")
w("  bromine diagonal-block defect; 'repaired' resets each diagonal block to minus the sum of its")
...
text = "\n".join(out) + "\n"
open(os.path.join(HERE, "hbr_hbr_classification.txt"), "w").write(text)
print(text)
