"""HF/HCl dimer: classification of the April 2026 CNEO-DFT runs (aug-cc-pV5Z excluded).
...
out = []
w = out.append
w("HF/HCl dimer, CNEO-DFT April 2026 runs: classification (aug-cc-pV5Z excluded)")
w("Masses: PySCF isotope-averaged (H 1.008, F 18.998, Cl 35.45). Atom order in every log: H0 F1 H2 Cl3; which H belongs to which molecule is read from the distances.")
w("")
w("PART 0: run status")
...
w("  bonding read from distances: hF = the H bonded to F, hCl = the H bonded to Cl. r(hF..Cl) = F-H...Cl hydrogen bond (HF donor),")
w("  r(hCl..F) = Cl-H...F (HCl donor); donor = the shorter of the two; angle = X-H...Y of that bond; tilt = H...Y-H at the acceptor;")
w("  'q' = which molecule's proton was quantum in the run (from the nuclear component index).")
w(f"{'start':<11}{'xc':<6}{'basis':<13}{'E (Eh)':>16} {'H-F':>6} {'H-Cl':>6} {'hF..Cl':>6} {'hCl..F':>6} {'F..Cl':>6} {'donor':>6} {'XH..Y':>6} {'tilt':>6} {'q':>4}  "
  f"{'F viol':>8} {'Cl viol':>8}  vibrations (cm^-1), 3N-6 = 6            imag  class")
class_summary = []
for start in ("global_min", "local_min"):
    for bas in BAS_ORDER:
        for xc in FUNC_ORDER:
            ...
            x = r["xyz"]
            F, Cl = 1, 3
            hF = 0 if dist(x[0], x[F]) < dist(x[2], x[F]) else 2      # the H covalently bound to F
            hCl = 2 if hF == 0 else 0
            rHF, rHCl = dist(x[hF], x[F]), dist(x[hCl], x[Cl])
            rHFCl, rHClF, rFCl = dist(x[hF], x[Cl]), dist(x[hCl], x[F]), dist(x[F], x[Cl])
            if rHFCl < rHClF:
                donor, a1, a2 = "HF", angle(x[F], x[hF], x[Cl]), angle(x[hF], x[Cl], x[hCl])
            else:
                donor, a1, a2 = "HCl", angle(x[Cl], x[hCl], x[F]), angle(x[hCl], x[F], x[hF])
            qidx = [int(k[1:]) for k in r["qnuc"]]
            q = "+".join(("HF" if i == hF else "HCl") for i in qidx) or "?"
            vib = r["res"]["vib"]
...
w("  chlorine (and fluorine) diagonal-block defect; 'repaired' resets each diagonal block to minus the sum of its")
...
text = "\n".join(out) + "\n"
open(os.path.join(HERE, "hf_hcl_classification.txt"), "w").write(text)
print(text)
