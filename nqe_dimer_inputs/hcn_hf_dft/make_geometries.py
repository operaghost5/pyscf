"""Extract the final HCN...HF geometries of the February 2026 conventional-DFT runs (aug-cc-pV5Z excluded)
...
directory (token 'cneodft') are ignored.  When a case has several logs the one that got furthest
(finished optimization, then most steps) is kept.  A log without a final geometry falls back to
...
STARTS = ["global_min", "local_min"]          # xlocal_min was never defined in the February inputs (DNE)
ATOMS = ["H", "C", "N", "H", "F"]
...
lines = []
missing = []
print(f"\n{'start':<11}{'xc':<6}{'basis':<13}{'E (Eh)':>16}  {'N..F':>6} {'N..H(F)':>8} {'H(C)..F':>8}  status / source")
for start in STARTS:
    for bas in BAS_ORDER:
        for xc in FUNC_ORDER:
            ...
            if rec is None:
                ...
                continue
            xyz = rec["xyz"]
            n_f = dist(xyz[2], xyz[4])
            n_h = dist(xyz[2], xyz[3])
            hc_f = dist(xyz[0], xyz[4])
            note = rec["status"]
            if rec["src"] != "summary":
                note += f"; geometry from {rec['src']}"
            print(f"{start:<11}{xc:<6}{bas:<13}{rec['energy']:>16.9f}  {n_f:6.3f} {n_h:8.3f} {hc_f:8.3f}  {note}")
            lines.append("5")
