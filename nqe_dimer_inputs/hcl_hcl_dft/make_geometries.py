"""Starting geometries for the (HCl)2 conventional-DFT re-optimization set (aug-cc-pV5Z excluded).
...
Two starts per functional and basis:
  ...
  c2h_saddle   a constructed C2h structure, the donor-acceptor interchange saddle point:
               ...
               arrangement, r(H-Cl) taken from the acceptor H-Cl of the April DFT geometry.
...
H0 (donor H), Cl1 (acceptor Cl), H2 (acceptor H), Cl3 (donor Cl); H0-Cl3 and Cl1-H2 are the
covalent bonds, H0...Cl1 the hydrogen bond.  If an April geometry has the donor at H2 the two
hydrogens are swapped and the comment line says so, the convention of ../hcl_hcl/.
"""
import glob
import math
import os
...
def donor_first(xyz):
    """Return (xyz with the donor H at index 0, swapped?).  H0-Cl3 / H2-Cl1 covalent, H0...Cl1 the H bond."""
    d = dist
    # covalent partner of each H: the nearer Cl
    p0 = 3 if d(xyz[0], xyz[3]) < d(xyz[0], xyz[1]) else 1
    p2 = 3 if d(xyz[2], xyz[3]) < d(xyz[2], xyz[1]) else 1
    if p0 == p2:
        raise ValueError("both hydrogens bonded to the same chlorine")
    other = {1: 3, 3: 1}
    ...
    if donor == 2:                                   # put the donor H at index 0
        ...
        swapped = True
        p0, p2 = p2, p0
    if p0 == 1:                                      # donor H bonded to Cl1: swap the chlorines so that Cl3 is the donor Cl
        x[1], x[3] = x[3], x[1]
        swapped = True
    return x, swapped
...
for path in sorted(glob.glob(os.path.join(LOGDIR, "*.log"))):
    ...
    if swapped:
        status += "; hydrogens/chlorines reordered so that H0-Cl3 donates to Cl1"
    ...
    for bas in BAS_ORDER:
        for xc in FUNC_ORDER:
            ...
            if start == "global_min":
                ...
                r_hf = dist(rec["xyz"][1], rec["xyz"][2])            # April DFT acceptor H-Cl length
                xyz = c2h_start(r_hf)
                comment = (f"{xc}/{bas} c2h_saddle E=n/a Eh constructed C2h interchange-saddle start: "
                           f"Cl..Cl {C2H_FF:.2f} A, r(H-Cl) {r_hf:.4f} A from the April DFT acceptor bond, "
