"""Starting geometries for the HF/HBr conventional-DFT re-optimization set (aug-cc-pV5Z excluded).
...
the April start label), the convention of ../hf_hbr/:

  hf_donor    F-H...Br-H, HF the donor, HBr roughly perpendicular   (April 'global_min' runs)
  hbr_donor   Br-H...F-H, HBr the donor, HF tilted                  (April 'local_min' runs)

Only the logs whose third file-name token is 'dft' are read.  Atom order everywhere: H0 F1 H2 Br3
(H0-F1 = HF, H2-Br3 = HBr); if a log has the hydrogens the other way round they are swapped and the
...
def canonical(xyz):
    """Return (xyz with H0 on F1 and H2 on Br3, donor symbol, swapped?)."""
    x = [row[:] for row in xyz]
    swapped = False
    if dist(x[0], x[1]) > dist(x[2], x[1]):          # H2 is the one on fluorine: swap the hydrogens
        x[0], x[2] = x[2], x[0]
        swapped = True
    hf_donates = dist(x[0], x[3]) < dist(x[2], x[1])   # H0...Br shorter than H2...F
    return x, ("F" if hf_donates else "Br"), swapped
...
for path in sorted(glob.glob(os.path.join(LOGDIR, "*.log"))):
    ...
    if swapped:
        status += "; hydrogens swapped so that H0-F1 and H2-Br3 are the covalent bonds"
