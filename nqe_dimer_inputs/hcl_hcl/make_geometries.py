"""Starting geometries for the (HCl)2 re-optimization set (aug-cc-pV5Z excluded).
...
Two starts per functional and basis:
  ...
  c2h_saddle   a constructed C2h structure, the donor-acceptor interchange saddle point:
               Cl...Cl = 3.85 A, both H-Cl bonds at 50 deg to the Cl...Cl axis in a trans
               arrangement, r(H-Cl) taken from the acceptor H-Cl of the April geometry.
...
Atom order everywhere: H0 (donor H), Cl1 (acceptor Cl), H2 (acceptor H), Cl3 (donor Cl);
H0-Cl3 and Cl1-H2 are the covalent bonds, H0...Cl1 the hydrogen bond. The April logs had
the donor at H2 (H2-Cl3 donating to Cl1), so the two hydrogens are swapped here; the donor
...
C2H_FF = 3.85          # Angstrom, Cl...Cl distance of the constructed saddle start
C2H_TILT_DEG = 50.0    # angle between each H-Cl bond and the Cl...Cl axis
...
def c2h_start(r_hf, R=C2H_FF, tilt_deg=C2H_TILT_DEG):
    """H0 Cl1 H2 Cl3 coordinates (Angstrom) of the trans-tilted C2h structure, inversion-symmetric."""
    th = math.radians(tilt_deg)
    fa = [-0.5 * R, 0.0, 0.0]                                   # donor Cl (atom 3)
    ha = [fa[0] + r_hf * math.cos(th), r_hf * math.sin(th), 0.0]  # donor H (atom 0), pointing at Cl1
    fb = [-fa[0], -fa[1], -fa[2]]                                # acceptor Cl (atom 1) = inversion of Cl3
...
for path in sorted(glob.glob(os.path.join(LOGDIR, "*.log"))):
    ...
    if not cycles or xyz is None:
        ...
        continue
    # units H0-Cl1 and H2-Cl3 (checked); put the DONOR unit on atoms 0 (H) and 3 (Cl), the acceptor on 2 (H) and 1 (Cl)
    assert dist(xyz[0], xyz[1]) < 1.6 and dist(xyz[2], xyz[3]) < 1.6, path
    swapped = False
    if dist(xyz[2], xyz[1]) < dist(xyz[0], xyz[3]):      # H2 donates to Cl1: H2 is the donor -> swap hydrogens
        xyz = [xyz[2], xyz[1], xyz[0], xyz[3]]           # now H0 (old H2) is bonded to Cl3 and points at Cl1
        swapped = True
    assert dist(xyz[0], xyz[3]) < 1.6 and dist(xyz[2], xyz[1]) < 1.6, path
...
lines = []
print(f"{'start':<11}{'xc':<6}{'basis':<13}{'E (Eh)':>16} {'Cl3-H0':>6} {'H0..Cl1':>7} {'Cl1-H2':>6} {'Cl1.Cl3':>7} {'H2..Cl3':>7}  note")
    ...
    for bas in BAS_ORDER:
        for xc in FUNC_ORDER:
            ...
            if start == "global_min":
                ...
                r_hf = dist(rec["xyz"][1], rec["xyz"][2])            # April acceptor H-Cl length
                xyz = c2h_start(r_hf)
                comment = (f"{xc}/{bas} c2h_saddle E=n/a Eh constructed C2h interchange-saddle start: "
                           f"Cl..Cl {C2H_FF:.2f} A, r(H-Cl) {r_hf:.4f} A from the April acceptor bond, "
